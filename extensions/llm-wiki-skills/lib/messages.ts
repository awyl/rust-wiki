...
export function buildCrystallizeDirective(skillPath: string, wikiName?: string | null): DirectiveMessage {
  ...
  return {
    ...
    content: [
      ...
      "",
      "1. Draft a compact extraction of this session's durable knowledge (2-6 items: decisions, findings, open questions — each with what, type, confidence, target slug). The worker cannot see this session, so this extraction is its only input.",
      "2. Run ONE command as a background bash task:",
      "",
      "```",
      ...
      "",
      "WORKER_PROMPT (pass to pi -p, handle quoting; include your extraction as EXTRACTED_KNOWLEDGE):",
      "```",
      "You are crystallizing a coding session you did not witness. EXTRACTED_KNOWLEDGE:",
      "<your extraction here>",
      "",
      "Read the crystallize skill at `" + skillPath + "` and follow it with these overrides:",
      "- Write wiki pages covering the EXTRACTED_KNOWLEDGE above: update the named slug when one fits, otherwise create a new page.",
      wikiScope,
